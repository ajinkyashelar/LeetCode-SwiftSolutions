...
class Solution {
    func stringShift(_ s: String, _ shift: [[Int]]) -> String {
        ...
        for i in shift {
            if i.count > 0 {
                shiftCount += i[0] == 0 ? -i[1] :  i [1]// Get count of difference in left and right
            }
        }
        if shiftCount > 0 {
            for _ in 0..<shiftCount {
                let range = string.startIndex..<string.index(before: string.endIndex)
                let endRange = string.index(before: string.endIndex)
                string = "\(string[endRange])\(string[range])"
            }
        }
        else {
            for _ in shiftCount..<0 {
                let range = string.index(after: string.startIndex)..<string.endIndex
                string = "\(string[range])\(string[string.startIndex])"
            }
        }
        return string
    }
