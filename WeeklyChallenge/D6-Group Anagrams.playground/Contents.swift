...
class Solution {
    func groupAnagrams(_ strs: [String]) -> [[String]] {
        ...
        var indexMapper : [String:Int] = [:]
        var group : [[String]] = []
        var groupIndex = -1
        
        for i in 0..<strs.count {
            if container.contains(String(strs[i].sorted()))
            {
                let index : Int = indexMapper[String(strs[i].sorted())] ?? groupIndex
                group[index].append(strs[i])
            }
            else {
                
                groupIndex = groupIndex + 1
                container.insert(String(strs[i].sorted()))
                indexMapper[String(strs[i].sorted())] = groupIndex
                group.append([strs[i]])
     ...
     func groupAnagramsHASH(_ strs: [String]) -> [[String]] {
        var hash: [String: [String]] = [:]
        
        for element in strs {
            let key = String(element.sorted())
            if var bucket = hash[key] {
